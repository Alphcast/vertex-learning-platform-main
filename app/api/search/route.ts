...
import { localDatasetSearch } from "@/lib/search/local-search";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const hasAIConfig =
      ...
      Boolean(process.env.SANITY_CONTEXT_MCP_URL);
    ...
    if (hasAIConfig) {
      ...
      const localResult = localDatasetSearch(query);
      hits = localResult.hits;
      reply = localResult.reply;
    }

    const results = await groundHits(hits, sort);
...
async function resolveIdentity({ distinctId, sessionId }: AnalyticsIdentity) {
  ...
  try {
    ...
    // Clerk is unconfigured or unavailable
  }
